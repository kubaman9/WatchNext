  ...
  return (
    <div className="mx-auto h-full max-w-xl overflow-y-auto px-5 py-5">
      ...
      <section className="mt-6 space-y-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 rounded-none border border-border bg-surface px-3 py-2 text-txt outline-none focus:border-accent"
          />
          <button onClick={saveKey} className="rounded-none bg-accent px-4 text-white">
            Save
          </button>
        </div>
      </section>
      ...
      <section className="mt-6 space-y-3">
        ...
        {!confirmReset ? (
          <button
            ...
            className="w-full rounded-none border border-red-900 bg-surface py-3 text-red-400 hover:border-red-600"
          >
            Reset taste profile
          </button>
        ) : (
          <div className="rounded-none border border-red-900 bg-surface p-3 text-sm">
            <p className="text-txt">Wipe Elo scores + genre weights and re-run onboarding?</p>
            <div className="mt-2 flex gap-2">
              <button onClick={onResetTaste} className="rounded-none bg-red-600 px-3 py-1 text-white">
                Reset
              </button>
              <button
      ...
      <section className="mt-6 space-y-2 border-t border-border pt-5">
        ...
        <button
          onClick={logout}
          className="w-full rounded-none border border-border bg-surface py-3 text-sub hover:border-accent hover:text-txt"
        >
          ⎋ Sign out
        </button>
      </section>
